...
function restart_opt {
    ...
    python $PFLOW/scripts/restart-g16.py $input_file
...
function update_existing_flow {
	...
	source_config
	cd $FLOW_TOOLS
	rsync -r --exclude=setup-flow.sh --exclude=test --exclude=functions.sh --exclude=LICENSE --exclude=README.md $PFLOW/* .
	cd $cur_dir
}
...
function extract_data {
	python $PFLOW/utils/data_extractor.py
}
...
# sets up vertical excitation energy calculation from geometry from given log file
# use: setup-vee <log file>
# effect: creates an input and sbatch file for a vee job and submits it
function setup_vee {
        source_config
	local log_file=$1
        local com_file="${log_file/.log/.com}"
        local route=$(grep '#p' $com_file | sed "s|M06|wB97xD|")
        local opt_keyword=$(echo $route | awk '/opt/' RS=" ")
        local charge=$(grep 'Charge =' $log_file | awk '{print $3}')
    local mult=$(grep 'Charge =' $log_file | awk '{print $6}')
        ...
        # setup sbatch
        cp $FLOW_TOOLS/templates/tddft_sbatch.txt $tddft.sbatch
        sed -i "s/JOBNAME/$vee/g" $tddft.sbatch
        sed -i "s/EMAIL/$DEFAULT_EMAIL/" $tddft.sbatch
        sed -i "s/TIME/$DFT_TIME/" $tddft.sbatch
        sed -i "s/PARTITION/$DEFAULT_PARTITION/" $tddft.sbatch
        sbatch $tddft.sbatch
...
function get_missing_input_files {
    ...
    if [[ "$curr_dir" == "$S0_SOLV" ]]; then
		...
		for file in *.pdb; do inchi="${file/.pdb/}"; charge=$(get_charge $inchi_key); bash $PFLOW/scripts/make-com.sh -i=$file -r='#p M06/6-31+G(d,p) SCRF=(Solvent=Acetonitrile) opt' -t=$inchi\_cat-rad_solv -c=$(($charge + 1)) -s=2 -l=$CAT_RAD_SOLV -f; rm $file; done
	elif [[ "$curr_dir" == "$RM1_D" ]]; then
		echo "UNSUPPORTED"
	elif [[ "$curr_dir" == "$SP_DFT" ]]; then
		for file in *.pdb; do inchi="${file/.pdb/}"; charge=$(get_charge $inchi_key); bash $PFLOW/scripts/make-com.sh -i=$file -r='#p M06/6-31+G(d,p)' -t=$inchi\_sp -l=$SP_DFT -c=$charge -f; rm $file; done
	fi
}
...
# use: update_flow
# effect: Updates $PFLOW directory to match GitHub repository
...
function setup_flow {
	bash $PFLOW/utils/setup_flow.sh "$@"
}


# temporary functions
function rerun_sp_tddft {
	bash $PFLOW/scripts/rerun_sp_tddft.sh
}

function rerun_s1_opts {
	bash $PFLOW/scripts/sn-xyz-extract.sh
}
